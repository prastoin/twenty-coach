...
- `packages/trainee-pwa/src` is layered: `lib/` wraps third-party or platform APIs (IndexedDB, the generated GraphQL client), `services/` holds everything that talks to the API or persists state (auth, session reads/writes, local store, sync, the local-first repository), `components/` holds the React tree. Components never call `lib/` directly.
